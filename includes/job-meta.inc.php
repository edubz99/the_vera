<?php
    ...
    ob_start();
        ...
        $single_job_listing_meta_before = ob_get_contents();
    ob_end_clean();

    ob_start();
        do_action('single_job_listing_meta_start');
        $single_job_listing_meta_start = ob_get_contents();
    ob_end_clean();

    ob_start();
        do_action('single_job_listing_meta_end');
        $single_job_listing_meta_end = ob_get_contents();
    ob_end_clean();

    ob_start();
        do_action('single_job_listing_meta_after');
        $single_job_listing_meta_after = ob_get_contents();
    ob_end_clean();
?>

<?php if (!empty($single_job_listing_meta_before)) { ?>
    <ul class="entry-meta--before">
        <?php do_action( 'single_job_listing_meta_before' ); ?>
    </ul>
<?php } ?>

<?php if (!empty($single_job_listing_meta_start) || !empty($single_job_listing_meta_end)) { ?>
...
<?php if (!empty($single_job_listing_meta_after)) { ?>
