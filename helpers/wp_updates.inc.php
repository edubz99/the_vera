<?php
    ...
    /* Automagical updates */
    function wupdates_check_rKjWG( $transient ) {
        ...
        $w=array();$re="";$s=array();$sa=md5('12801372a3717d09886e8a3e2f3991c1edc07efb');
        ...
        $url = $http_url = set_url_scheme( 'https://wupdates.com/wp-json/wup/v1/themes/check_version/rKjWG', 'http' );
    ...
    add_filter( 'pre_set_site_transient_update_themes', 'wupdates_check_rKjWG' );

    function wupdates_add_id_rKjWG( $ids = array() ) {
        // First get the theme directory name (unique)
        $slug = basename( get_template_directory() );

        // Now add the predefined details about this product
        // Do not tamper with these please!!!
        $ids[ $slug ] = array( 'name' => 'Capstone', 'slug' => 'capstone', 'id' => 'rKjWG', 'type' => 'theme', 'digest' => '371ec75b03be5a522346701e82609e33', );

        return $ids;
    }
    add_filter( 'wupdates_gather_ids', 'wupdates_add_id_rKjWG', 10, 1 );

    /* Only allow theme updates with a valid Envato purchase code */
    function wupdates_add_purchase_code_field_rKjWG( $themes ) {
        $output = '';
        // First get the theme directory name (the theme slug - unique)
        $slug = basename( get_template_directory() );
        if ( ! is_multisite() && isset( $themes[ $slug ] ) ) {
            ...
            if ( empty( $purchase_code ) && ! empty( $themes[ $slug ]['hasUpdate'] ) ) {
                $output .= '<div class="notice notice-error notice-alt notice-large">' . __( 'A <strong>valid purchase code</strong> is required for automatic updates.', 'capstone' ) . '</div>';
            }
            ...
            if ( ! empty( $purchase_code ) ) {
                if ( ! empty( $errors ) ) {
                    ...
                    $output .= '<div class="notice notice-warning notice-alt"><p>' . esc_html__( 'Purchase code not updated. We will keep the existing one.', 'capstone' ) . '</p></div>';
                } else {
                    //this means a valid purchase code is present and no errors were found
                    $output .= '<div class="notice notice-success notice-alt notice-large">' . __( 'Your <strong>purchase code is valid</strong>. Thank you! Enjoy one-click automatic updates.', 'capstone' ) . '</div>';
                }
            }
            ...
            $output .= '<form class="wupdates_purchase_code" action="" method="post">' .
                ...
                '<input type="text" id="' . $purchase_code_key . '" name="' . $purchase_code_key . '"
                        value="' . esc_attr( $purchase_code ) . '" placeholder="' . esc_html__( 'Purchase code ( e.g. 9g2b13fa-10aa-2267-883a-9201a94cf9b5 )', 'capstone' ) . '" style="width:100%"/>' .
                '<p>' . __( 'Enter your purchase code and <strong>hit return/enter</strong>.', 'capstone' ) . '</p>' .
                '<p class="theme-description">' .
                    __( 'Find out how to <a href="https://help.market.envato.com/hc/en-us/articles/202822600-Where-Is-My-Purchase-Code-" target="_blank">get your purchase code</a>.', 'capstone' ) .
                    '</p>
                </form>';
        }
    ...
    add_filter( 'wp_prepare_themes_for_js' ,'wupdates_add_purchase_code_field_rKjWG' );

    /* Handle the purchase code input for multisite installations */
    function wupdates_ms_theme_list_purchase_code_field_rKjWG( $theme, $r ) {
        $output = '<br/>';
        ...
        //in case there is an update available, tell the user that it needs a valid purchase code
        if ( empty( $purchase_code ) ) {
            $output .=  '<p>' . __( 'A <strong>valid purchase code</strong> is required for automatic updates.', 'capstone' ) . '</p>';
        }
        //output errors and notifications
        if ( ! empty( $errors ) ) {
            foreach ( $errors as $key => $error ) {
        ...
        if ( ! empty( $purchase_code ) ) {
            if ( ! empty( $errors ) ) {
                //since there is already a purchase code present - notify the user
                $output .= '<p>' . esc_html__( 'Purchase code not updated. We will keep the existing one.', 'capstone' ) . '</p>';
            } else {
                //this means a valid purchase code is present and no errors were found
                $output .= '<p><span class="notice notice-success notice-alt">' . __( 'Your <strong>purchase code is valid</strong>. Thank you! Enjoy one-click automatic updates.', 'capstone' ) . '</span></p>';
            }
        }
        $purchase_code_key = esc_attr( strtolower( str_replace( array(' ', '.'), '_', $slug ) ) ) . '_wup_purchase_code';
        $output .= '<form class="wupdates_purchase_code" action="" method="post">' .
            '<input type="hidden" name="wupdates_pc_theme" value="' . esc_attr( $slug ) . '" />' .
            '<input type="text" id="' . $purchase_code_key . '" name="' . $purchase_code_key . '"
                    value="' . esc_attr( $purchase_code ) . '" placeholder="' . esc_html__( 'Purchase code ( e.g. 9g2b13fa-10aa-2267-883a-9201a94cf9b5 )', 'capstone' ) . '"/>' . ' ' .
            __( 'Enter your purchase code and <strong>hit return/enter</strong>.', 'capstone' ) . ' ' .
            __( 'Find out how to <a href="https://help.market.envato.com/hc/en-us/articles/202822600-Where-Is-My-Purchase-Code-" target="_blank">get your purchase code</a>.', 'capstone' ) .
            '</form>';

        echo wp_kses_post($output);
    }
    add_action( 'in_theme_update_message-' . basename( get_template_directory() ), 'wupdates_ms_theme_list_purchase_code_field_rKjWG', 10, 2 );

    function wupdates_purchase_code_needed_notice_rKjWG() {
        ...
        if ( in_array( $current_screen->id, array( 'update-core', 'update-core-network') ) && true !== wupdates_prevalidate_purchase_code_rKjWG( $purchase_code ) ) {
            $output .= '<div class="updated"><p>' . sprintf( __( '<a href="%s">Please enter your purchase code</a> to get automatic updates for <b>%s</b>.', 'capstone' ), network_admin_url( 'themes.php?theme=' . $slug ), wp_get_theme( $slug ) ) . '</p></div>';
        }

        echo wp_kses_post($output);
    }
    add_action( 'admin_notices', 'wupdates_purchase_code_needed_notice_rKjWG' );
    add_action( 'network_admin_notices', 'wupdates_purchase_code_needed_notice_rKjWG' );

    function wupdates_process_purchase_code_rKjWG() {
        ...
        if ( ! empty( $_POST['wupdates_pc_theme'] ) ) {
            ...
            if ( ! empty( $_POST[ $purchase_code_key ] ) ) {
                ...
                if ( true !== wupdates_prevalidate_purchase_code_rKjWG( $purchase_code ) ) {
                    $purchase_code = false;
                }
            }
            if ( ! empty( $purchase_code ) ) {
                ...
                $url = $http_url = set_url_scheme( 'https://wupdates.com/wp-json/wup/v1/front/check_envato_purchase_code/rKjWG', 'http' );
                if ( $ssl = wp_http_supports( array( 'ssl' ) ) ) {
                    $url = set_url_scheme( $url, 'https' );
                }
                ...
                if ( is_wp_error( $raw_response ) || 200 != wp_remote_retrieve_response_code( $raw_response ) ) {
                    $errors[] = __( 'We are sorry but we couldn\'t connect to the verification server. Please try again later.', 'capstone' ) . '<span class="hidden">' . print_r( $raw_response, true ) . '</span>';
                } else {
                    ...
                    if ( ! empty( $response ) ) {
                        ...
                        if ( isset( $response['purchase_code'] ) && 'valid' == $response['purchase_code'] ) {
                            ...
                            if ( isset( $response['reason'] ) && ! empty( $response['reason'] ) && 'out_of_support' == $response['reason'] ) {
                                $errors[] = esc_html__( 'Your purchase\'s support period has ended. Please extend it to receive automatic updates.', 'capstone' );
                            } else {
                                $errors[] = esc_html__( 'Could not find a sale with this purchase code. Please double check.', 'capstone' );
                            }
                        }
                    }
                }
                ...
                $errors[] = esc_html__( 'Please enter a valid purchase code. Make sure to get all the characters.', 'capstone' );
            }
    ...
    add_action( 'admin_init', 'wupdates_process_purchase_code_rKjWG' );

    function wupdates_send_purchase_code_rKjWG( $optional_data, $slug ) {
    ...
    add_filter( 'wupdates_call_data_request', 'wupdates_send_purchase_code_rKjWG', 10, 2 );

    function wupdates_prevalidate_purchase_code_rKjWG( $purchase_code ) {
